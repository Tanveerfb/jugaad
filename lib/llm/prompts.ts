...
export function buildPlanAgentSystemPrompt(stack: StackConfig): string {
  ...
  const isNextjs = stack.selected.includes("nextjs");
  const frameworkDesc = isNextjs
    ? "Next.js web application (App Router, SSR, full-stack)"
    : "React.js single-page application (Vite, client-only)";

  // Auth strategies available depend on the framework
  const authStrategyType = isNextjs
    ? `"none" | "nextauth" | "firebase" | "custom"`
    : `"none" | "firebase" | "custom"`;

  return `You are an expert web application planning assistant for a local-first AI scaffolding studio called Jugaad.

Your role is to help the user plan their ${frameworkDesc} and produce a structured JSON plan.
...
export function buildTaskGeneratorPrompt(
  ...
  const sel = plan.stack.selected;
  const isNextjs = sel.includes("nextjs");
  const hasTS = sel.includes("typescript");
  const hasTailwind = sel.includes("tailwind");
  const hasBootstrap = sel.includes("bootstrap");
  const hasMUI = sel.includes("mui");
  const hasZustand = sel.includes("zustand");
  const hasRHF = sel.includes("rhf");

  // ── Persona & init command ────────────────────────────────────────────────
  const persona = isNextjs
    ? "You are a principal-level Next.js architect and senior TypeScript engineer."
    : "You are a senior React engineer specialising in Vite SPA development with JavaScript.";

  const initCommand = isNextjs
    ? "`npx create-next-app@latest`"
    : "`npm create vite@latest . -- --template react` (plain JavaScript, no TypeScript template)";

  // ── File structure conventions ────────────────────────────────────────────
  const structureRules = isNextjs
...
6. The project is always initialized with ${initCommand} before your generated files are written. Therefore, do NOT generate tsconfig.json or postcss.config.mjs — they are pre-configured correctly. Start tasks in this order: package.json (with all stack-specific deps) → next.config.ts (only if stack needs custom config) → app/globals.css → types → utils → components → pages.`
    : `3. File paths use Vite SPA convention with a src/ prefix (e.g. "src/pages/Dashboard.jsx", "src/components/Navbar.jsx"). All source files use .jsx extension (not .tsx — this is a JavaScript project).
4. Every page component must be a default export React function component.
5. No API route files — this is a purely client-side app. Server calls go to external APIs via fetch/axios.
6. The project is always initialized with ${initCommand} before your generated files are written. Start tasks in this order: package.json (with all stack-specific deps) → vite.config.js (set up @/ alias pointing to src/) → index.html → src/main.jsx (entry point + router + global CSS imports) → src/App.jsx (route definitions) → types/utils → components → pages.`;

  // ── Styling rules ─────────────────────────────────────────────────────────
  const stylingRules = hasTailwind
   ...
   LAYOUT RELIABILITY: For fixed-grid UIs (game boards, calendars, data grids), use CSS Grid with explicit row/column counts. To make a grid container perfectly square, use the padding-bottom trick: set the outer wrapper to position:relative + paddingBottom:"100%", and make the inner grid div position:absolute with inset:0.`
    : hasBootstrap
      ? `9. BOOTSTRAP 5: Import Bootstrap CSS in src/main.jsx as \`import 'bootstrap/dist/css/bootstrap.min.css'\`. Do NOT generate a custom CSS framework config. Use Bootstrap utility classes (e.g. "container", "row", "col-md-6", "btn btn-primary", "mt-3", "d-flex") directly on JSX elements via className. For Bootstrap JS components (modals, dropdowns, tooltips), use react-bootstrap components from "react-bootstrap" — do NOT import Bootstrap's bundled JS. Avoid inline styles except for truly dynamic values.`
      : `9. No CSS framework configured — use plain CSS modules or inline styles.`;

  // ── Framework-specific runtime rules ──────────────────────────────────────
  const frameworkRules = isNextjs
   ...
   CRITICAL: Files in utils/, lib/, or stores/ are pure TypeScript modules — they must NEVER use React hooks. Only components (components/, app/) and custom hooks (hooks/) may use React hooks.
11. next.config.ts: Keep MINIMAL. Do NOT set typescript.ignoreBuildErrors or eslint.ignoreDuringBuilds — the app must produce zero errors and zero warnings.
...
17. Prefer React Server Components for data fetching. Only use "use client" when genuine interactivity is required.`
    : `10. NO "use client" / "use server" directives — this is a plain React+Vite SPA. All components are client-side by default.
11. vite.config.js: Configure the @/ path alias: \`import path from "path"; resolve: { alias: { "@": path.resolve("./src") } }\`. This requires "path" from Node (available in Vite config). Do NOT add TypeScript options.
12. No tsconfig.json strict-mode rules apply — this is a JavaScript project. ESLint with eslint-plugin-react is sufficient.
13. IMPORT ALIAS: All generated files use "@/" for local imports (resolves to src/). Configure this alias in vite.config.js.
14. ROUTING: Use react-router-dom v6 for all navigation. Use <BrowserRouter> in src/main.jsx, <Routes>/<Route> in src/App.jsx, and <Link>/<NavLink> for navigation. Use useNavigate() and useParams() hooks in components.
15. Every component or page doing async work must handle loading AND error states. No indeterminate UI states.
16. Design quality: Clean, minimal, polished UI. Real content — no lorem ipsum. Consistent Bootstrap spacing (mt-3, p-4, gap-3). Semantic HTML. Responsive using Bootstrap grid (col-sm/md/lg). Accessible: labels, aria attributes, keyboard navigation.
17. All data fetching happens inside useEffect or custom hooks — no server-side fetching concepts.`;

  // ── Version context ───────────────────────────────────────────────────────
  const versionRules = isNextjs
   ...
   State: ${hasZustand ? `zustand@${LATEST_VERSIONS["zustand"]}` : ""}${hasRHF ? `, react-hook-form@${LATEST_VERSIONS["react-hook-form"]}` : ""}
   ...
   • Firebase v12: modular SDK only — \`import { getAuth } from "firebase/auth"\`. Never \`firebase/compat\`.`
    : `8. PACKAGE VERSIONS AND BREAKING API CHANGES — task instructions MUST reflect these exact APIs:
   Core: vite@latest, react@${LATEST_VERSIONS["react"]}, react-dom@${LATEST_VERSIONS["react-dom"]} — JavaScript (no TypeScript).
   ${hasBootstrap ? `Bootstrap 5: bootstrap@^5.3 (CSS + components via react-bootstrap). Import 'bootstrap/dist/css/bootstrap.min.css' in src/main.jsx.` : ""}
   ${hasMUI ? `MUI: @mui/material@latest, @emotion/react, @emotion/styled (all required peer deps). Use ThemeProvider in src/main.jsx.` : ""}
   Routing: react-router-dom@^6 — use createBrowserRouter or <BrowserRouter>/<Routes>/<Route>.
   ${hasZustand ? `Zustand v5: \`create()(set => ...)\` (double-call, no TypeScript generics needed in JS). import { create } from "zustand".` : ""}
   ${hasRHF ? `react-hook-form@${LATEST_VERSIONS["react-hook-form"]} — \`useForm()\`, \`register\`, \`handleSubmit\`, \`formState: { errors }\`. No zodResolver needed (JavaScript project).` : ""}
   Firebase v12: modular SDK only — \`import { getAuth } from "firebase/auth"\`. Never \`firebase/compat\`.`;

  // ── Auth pages ────────────────────────────────────────────────────────────
  const authRules = isNextjs
    ...
    : `19. AUTH PAGES (MANDATORY): If authStrategy is not "none", you MUST generate ALL of the following — never omit them:
    - src/pages/Login.jsx — login form component with email + password fields
    - src/pages/Signup.jsx — signup form component for email/password registration
    - src/components/ProtectedRoute.jsx — wrapper component that checks auth state and redirects unauthenticated users to /login using useNavigate()
    - src/lib/firebase.js — Firebase app init + auth/firestore exports (for Firebase auth strategy)
    - Wrap protected <Route> elements with <ProtectedRoute> in src/App.jsx
    No server-side middleware exists in a Vite SPA — auth protection is entirely client-side.
    The user explicitly chose authentication — missing auth pages make the app non-functional.`;

  return `${persona} Decompose the following project plan into a perfectly ordered list of file generation tasks. Each file will be generated by an LLM so every instruction must be precise, complete, and unambiguous.
...
${structureRules}
7. Include ALL necessary files: config, types/constants, utils, stores, components, and pages. Do not omit any file the app needs to run.
${versionRules}
${stylingRules}
${frameworkRules}
...
20. FILE SIZE LIMIT — CRITICAL: Every generated file must be ≤120 lines. This is non-negotiable because each file is written by an LLM with a limited context window.
    - If a module (utility, store, component) would exceed ~120 lines, SPLIT it into multiple focused files. Add a barrel index.${hasTS ? "ts" : "js"}/index.${hasTS ? "tsx" : "jsx"} that re-exports everything.
    - Components: split by logical section (header, body, footer) or by sub-component.
    - Stores: split by domain — never one giant store file.
...
21. TYPE LITERAL VERBATIM — CRITICAL: When a type/constant definition uses exact string literal values (e.g. \`const STATUS = { ONGOING: 'ongoing', DONE: 'done' }\`), ALL implementation files that produce or consume those values MUST use the exact strings verbatim. Task instructions for consuming files MUST call out the exact values.
...
[
  {
    ...
    "filePath": "relative/path/to/file.${hasTS ? "tsx" : "jsx"}",
...
export function buildTaskExecutorPrompt(
  ...
  // When stackSelected is empty, fall back to Next.js behaviour (backward compat).
  const isNextjs =
    stackSelected.length === 0 || stackSelected.includes("nextjs");
  const hasTailwind =
    stackSelected.length === 0 || stackSelected.includes("tailwind");
  const hasBootstrap = stackSelected.includes("bootstrap");
  const hasTS =
    stackSelected.length === 0 || stackSelected.includes("typescript");
...
- zustand: "${LATEST_VERSIONS["zustand"]}" (if used)\n`;
    } else {
      fileSpecificRules = `\nCRITICAL VERSION REQUIREMENTS (use exactly these — no older versions):
- vite: "^6", @vitejs/plugin-react: "^4" (devDeps)
- react: "${LATEST_VERSIONS["react"]}", react-dom: "${LATEST_VERSIONS["react-dom"]}"
- react-router-dom: "^6"
${hasBootstrap ? `- bootstrap: "^5.3", react-bootstrap: "^2" (both required for Bootstrap 5)\n` : ""}- Do NOT include TypeScript, tailwindcss, or @tailwindcss/postcss — this is a plain JavaScript + ${hasBootstrap ? "Bootstrap" : "CSS"} project.
- zustand: "${LATEST_VERSIONS["zustand"]}" (if used)\n`;
    }
...
- "jsx": "preserve", "moduleResolution": "bundler" — required for Next.js App Router.\n`;
    ...
    const moduleLabel = hasTS ? "TypeScript" : "JavaScript";
    fileSpecificRules = `\n⚠ PURE ${moduleLabel.toUpperCase()} FILE — CRITICAL CONSTRAINTS:
This file lives in ${task.filePath.split("/")[0]}/ and is a plain ${moduleLabel} module.
...
${hasTS ? 'DO NOT import from "react" at all (unless importing only types like React.FC — but prefer not to).' : ""}
Write ONLY plain exported ${moduleLabel} functions, types, constants, and classes.
...
Pure functions are strongly preferred — avoid side effects.\n`;
  ...
  const isUIFile = isNextjs
    ? /^(app|components|pages)\//.test(task.filePath)
    : /^src\/(pages|components|views)\//.test(task.filePath);

  const strictModeRules = hasTS
...
• TYPE UNION AS VALUE: Never write \`someKey: 'a' | 'b'\` inside an object literal — that is runtime bitwise OR (evaluates to 0). Type union syntax is only valid in \`type\`/\`interface\`/parameter declarations.`
    : `CODE QUALITY:
• No unused variables or imports — keep the file clean.
• No console.log in production code (use a logger or remove).
• No @ts-ignore or eslint-disable comments.
• TYPE LITERAL VERBATIM: When dependency files define a constant enum or status map (e.g. \`const STATUS = { DONE: 'done' }\`), use those EXACT strings as runtime values everywhere — never invent abbreviations.`;

  const useClientRule = isNextjs
    ...
    : `// Vite SPA — no "use client" / "use server" directives anywhere.`;

  const importRule = isNextjs
    ? `IMPORTS: "@/" for all local imports. "next/navigation" not "next/router". Order: React/Next → third-party → "@/" local.`
    : `IMPORTS: "@/" for all local imports (resolves to src/). Use react-router-dom hooks (useNavigate, useParams, useLocation) — never next/navigation. Order: React → third-party → "@/" local.`;

  const qualityRules = `
You are a ${hasTS ? "principal-level TypeScript/React" : "senior JavaScript/React"} engineer. Write clean, production-quality code. No shortcuts, no placeholder code, no suppression comments.
...
UI QUALITY:
• Semantic HTML. Never <div onClick> — use <button type="button">. Form inputs need <label>.
• Responsive using Bootstrap grid (col-sm/col-md/col-lg). One <h1> per page. Show loading + error states. Unique stable keys on lists.
• Bootstrap 5 utility classes for spacing (mt-3, p-4, gap-3), display (d-flex, d-none), and typography (text-muted, fw-bold). Use react-bootstrap components (Button, Form, Modal, Card, Navbar) instead of raw Bootstrap JS.
• For dynamic CSS class toggling use a plain ternary: className={isActive ? "btn btn-primary" : "btn btn-outline-secondary"}.
• LAYOUT RELIABILITY: For fixed-grid UIs, prefer CSS Grid with explicit row/column counts in an inline style (display:"grid", gridTemplateColumns:"repeat(N, 1fr)") since Bootstrap grid is 12-column flexbox and may not suit all layouts.
`
        : `
UI QUALITY:
• Semantic HTML. Never <div onClick> — use <button type="button">. Form inputs need <label>.
• Responsive design. One <h1> per page. Show loading + error states. Unique stable keys on lists.
`
    : ""
}`;
  ...
  const isPureLibUtil = /^(lib|src\/lib)\/.*\.(ts|js)$/.test(task.filePath);

  let versionContext = "";
  if (isCodeFile && !isPureLibUtil) {
    if (isNextjs) {
...
`;
      ...
      versionContext = `\nINSTALLED PACKAGE VERSIONS — use the API that matches these exact versions:
• vite@^6 + @vitejs/plugin-react — Vite SPA. No SSR. No next/navigation — use react-router-dom instead.
• react@${LATEST_VERSIONS["react"]}, react-dom@${LATEST_VERSIONS["react-dom"]} — React 19. \`useActionState\`, \`use()\` hook, \`useOptimistic\`. ref is now a prop (no forwardRef needed). useTransition for async UI.
• react-router-dom@^6 — \`<BrowserRouter>\`, \`<Routes>\`, \`<Route>\`, \`<Link>\`, \`<NavLink>\`, \`useNavigate()\`, \`useParams()\`, \`useLocation()\`, \`<Navigate>\` for redirects.
${
  hasBootstrap
    ? `• bootstrap@^5.3 — import 'bootstrap/dist/css/bootstrap.min.css' in src/main.jsx. Use Bootstrap utility classes via className. For Bootstrap JS components (modals, tooltips), use react-bootstrap.
• react-bootstrap@^2 — React components wrapping Bootstrap 5 JS: Button, Form, Modal, Navbar, Card, Dropdown, Spinner, Alert. Import from "react-bootstrap".
`
    : ""
}• zustand@${LATEST_VERSIONS["zustand"]} — Zustand v5. JS usage: \`import { create } from "zustand"\`; \`const useMyStore = create((set) => ({ ... }))\` (no TypeScript generics). Persist: \`import { persist } from "zustand/middleware"\`.
• firebase@${LATEST_VERSIONS["firebase"]} — Firebase v12 modular SDK. \`import { getAuth, signInWithEmailAndPassword } from "firebase/auth"\`. \`import { getFirestore, doc, getDoc } from "firebase/firestore"\`. Never import from "firebase/compat/*".
• react-hook-form@${LATEST_VERSIONS["react-hook-form"]} — v7. JS usage: \`const { register, handleSubmit, formState: { errors } } = useForm()\`. No zodResolver needed in JavaScript.
• framer-motion@${LATEST_VERSIONS["framer-motion"]} — v12. \`motion.div\`, \`AnimatePresence\`, \`useAnimate()\`, \`useMotionValue()\`. \`variants\` for orchestrated animations.
`;
    }
  }
...
export function buildFixerPrompt(
  ...
  const mandatoryVersions = isNextjs
...
- react: "${LATEST_VERSIONS["react"]}"
- react-dom: "${LATEST_VERSIONS["react-dom"]}"
- vite: "^6" (devDependency)
- @vitejs/plugin-react: "^4" (devDependency)
- react-router-dom: "^6"
${hasBootstrap ? `- bootstrap: "^5.3"\n- react-bootstrap: "^2"` : ""}

RULES:
- Keep "scripts": { "dev": "vite", "build": "vite build", "preview": "vite preview" }
- Put vite and @vitejs/plugin-react in devDependencies
- Do NOT include typescript, tailwindcss, or @tailwindcss/postcss — this is a JavaScript project`;

  return `You are auditing a generated ${isNextjs ? "Next.js" : "React+Vite"} project. Your ONLY job is to output a corrected package.json.
...
export function buildTypecheckFixPrompt(
  ...
  return `${buildTaskExecutorPrompt(task, dependencyFileContents, stackSelected)}
